...
Linux Chromium’s `distribution.require_eula=false` suppresses its empty unbranded additional-terms dialog; normal account choice, first-run preferences and bookmark import remain enabled. Only portable data is retained. Raspberry-patched Chromium binaries, hardware video flags and automatic extension downloads are omitted; codecs and sandboxing use Alpine Chromium. Labwc sessions enable native Wayland and Wayland IME for the touch keyboard. The Chromium sandbox is not disabled.
